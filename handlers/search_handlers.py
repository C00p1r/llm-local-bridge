...
def register_search_handlers():
    ...
    async def _handle_search_codebase(params: Dict[str, Any]):
        ...
        return await asyncio.to_thread(search_ops.search_codebase, query, path=path, include_pattern=include_pattern, max_results=max_results)
    ...
    async def _handle_find_references(params: Dict[str, Any]):
        ...
        return await asyncio.to_thread(search_ops.find_references, symbol, file_type=file_type, scope_dir=scope_dir)
